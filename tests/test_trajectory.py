...
class TestTrajectoryStore:
    ...
    def test_search_trajectories(self):
        ...
        search_resp = json.dumps({
            ...
            "results": [
                {"key": "trajectory/t-1", "value": json.dumps({
                    "goal": "implement add function",
                    "steps": ["explore", "write tests", "write code"],
                    "outcome": "success",
                }), "similarity": 0.85},
                {"key": "trajectory/t-2", "value": json.dumps({
    ...
    def test_format_context_for_agent(self):
        ...
        matches = [
            TrajectoryMatch(
                key="t-1", goal="implement add function",
                steps=["explore", "write tests", "write code"],
                outcome="success", similarity=0.85, duration=12.5,
                tech_stack=["python"],
            ),
        ]
    ...
    def test_store_and_search_roundtrip(self):
        ...
        search_resp = json.dumps({
            "query": "build API",
            "results": [{"key": "trajectory/t-api", "value": json.dumps(stored_data), "similarity": 0.92}],
            "total": 1,
        })
