...
class TestPipelineTracing(unittest.TestCase):
    ...
    def test_orchestrator_has_all_stage_traces(self):
        ...
        code = src.read_text()
        # All 5 trace_generation calls (intent-gate, planner, reviewer, executor/delegator, synthesizer)
        assert 'trace_generation("intent-gate"' in code
        assert 'trace_generation("planner"' in code
        assert 'trace_generation("reviewer"' in code
        assert 'trace_generation("synthesizer"' in code
        # Executor or delegator (depends on local mode)
        assert 'stage5_name' in code
        assert 'trace_generation(stage5_name' in code
